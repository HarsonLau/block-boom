  ...
  with HasBoomFTBParameters
{
  ...
  when ((s2_valid && !icache.io.resp.valid) ||
    ...
    when (s1_valid && s1_vpc === n_f2_predicted_target && !n_f2_correct_f1_ghist) {
  ...
  val n_f3_ftb_entry = withReset(reset.asBool || f3_clear) {
    Module(new Queue(new FTBEntry, 1, pipe=true, flow=true)) }
  ...
  val f3_ftb_entry = n_f3_ftb_entry.io.deq.bits
  val f3_pftAddr = f3_ftb_entry.pftAddr
  val needFallThrough = n_f3_bpd_resp.io.deq.bits.pred.hit && f3_ftb_entry.valid &&
  f3_pftAddr =/= 0.U && f3_pftAddr =/= 4.U && f3_ftb_entry.isFull && !f3_ftb_entry.carry &&
  ...
  f3_pd.jalTarget := f3_targs(f3_jmp_idx)
  f3_pd.rvcMask := f3_is_rvc
  ...
  when (f3.io.deq.valid && f4_ready) {
    ...
    when (f3_redirects.reduce(_||_) || needFallThrough) {
      f3_prev_is_half := false.B
    }
  ...
  val use_pd_gen = (f3_btb_mispredicts.reduce(_||_) && enableBTBFastRepair.B) || (f3_pd.jmpInfo.valid && !n_f3_ftb_entry.io.deq.bits.valid) 
  ...
  // f4_btb_corrections.io.enq.bits.pd                   := DontCare
