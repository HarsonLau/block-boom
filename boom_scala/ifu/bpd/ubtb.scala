...
class MicroBTBBranchPredictorBank(params: BoomMicroBTBParams)(implicit p: Parameters) extends BranchPredictorBank()(p)
{
  ...
  class MicroBTBPredictMeta extends Bundle {
    val ctrs  = Vec(BPBankWidth, UInt(2.W))
  }
  ...
  val meta     = SyncReadMem(nSets, Vec(BPBankWidth, UInt(btbMetaSz.W)))
  val btb      = SyncReadMem(nSets, Vec(BPBankWidth, UInt(btbEntrySz.W)))

  val mems = Seq(("ubtb_meta", nSets, BPBankWidth * btbMetaSz),
                 ("ubtb_data", nSets, BPBankWidth * btbEntrySz))
  ...
  val wrbypass         = Reg(Vec(nWrBypassEntries, Vec(BPBankWidth, new MicroBTBMeta)))
  ...
  val s1_update_wmeta_data = Wire(Vec(BPBankWidth, new MicroBTBMeta))
  for (w <- 0 until BPBankWidth) {
