...
class FauFTBImpl(implicit p: Parameters) extends BoomModule()(p) with FauFTBParams with BPUUtils {
  ...
  val raw_ctr = Mux1H(s1_hit_oh, VecInit((0 until numWays).map(w => VecInit((0 until numBr).map(b => ctrs(w)(b))))))
  ...
  val u_s0_hit_way = Wire(UInt(log2Ceil(numWays).W))
...
class FauFTB(implicit p: Parameters) extends BlockPredictorBank with FauFTBParams {
  ...
  val impl = Module(new FauFTBImpl)
  ...
  impl.io.update_pc.valid := u.valid
  impl.io.update_pc.bits := io.update.bits.pc
  impl.io.update_entry := io.update.bits.ftb_entry
  impl.io.update_meta := io.update.bits.meta.asTypeOf(new FTBMeta)
  impl.io.is_commit_update := io.update.bits.is_commit_update
  impl.io.is_btb_mispredict_update := io.update.bits.is_btb_mispredict_update
  impl.io.br_taken_mask := io.update.bits.br_taken_mask
  impl.io.br_update_mask := u_s0_br_update_valids
  ...
  val s1_ftb_entry = impl.io.resp_entry
  ...
  hit_pred.br_taken_mask := impl.io.resp_br_taken
  ...
  for ( i <- 0 until numBr){
    ...
    io.resp.f1.perfs(i).fauftb_taken := impl.io.resp_br_taken(i)
