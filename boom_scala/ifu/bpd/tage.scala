  ...
  with HasBoomFTBParameters
{
  ...
  val table  = SyncReadMem(nRows, Vec(nColumns, UInt(tageEntrySz.W)))

  val mems = Seq((f"tage_l$histLength", nRows, nColumns * tageEntrySz))
  ...
  table.write(
    ...
    Mux(doing_reset, VecInit(Seq.fill(nColumns) { 0.U(tageEntrySz.W) }), VecInit(update_wdata.map(_.asUInt))),
    Mux(doing_reset, ~(0.U(nColumns.W))                                , io.update_mask.asUInt).asBools
  )

  val update_hi_wdata = Wire(Vec(nColumns, Bool()))
  hi_us.write(
    Mux(doing_reset, reset_idx, Mux(doing_clear_u_hi, clear_u_idx, update_idx)),
    Mux(doing_reset || doing_clear_u_hi, VecInit((0.U(nColumns.W)).asBools), update_hi_wdata),
    Mux(doing_reset || doing_clear_u_hi, ~(0.U(nColumns.W)), io.update_u_mask.asUInt).asBools
  )

  val update_lo_wdata = Wire(Vec(nColumns, Bool()))
  lo_us.write(
    Mux(doing_reset, reset_idx, Mux(doing_clear_u_lo, clear_u_idx, update_idx)),
    Mux(doing_reset || doing_clear_u_lo, VecInit((0.U(nColumns.W)).asBools), update_lo_wdata),
    Mux(doing_reset || doing_clear_u_lo, ~(0.U(nColumns.W)), io.update_u_mask.asUInt).asBools
  )

  val wrbypass_tags    = Reg(Vec(nWrBypassEntries, UInt(tagSz.W)))
  val wrbypass_idxs    = Reg(Vec(nWrBypassEntries, UInt(log2Ceil(nRows).W)))
  val wrbypass         = Reg(Vec(nWrBypassEntries, Vec(nColumns, UInt(3.W))))
...
class TageBranchPredictorBank(params: BoomTageParams = BoomTageParams())(implicit p: Parameters) extends BranchPredictorBank()(p)
{
  ...
  val s1_update_mispredict_mask = UIntToOH(s1_update.bits.cfi_idx.bits) &
    Fill(nColumns, s1_update.bits.cfi_mispredicted)
