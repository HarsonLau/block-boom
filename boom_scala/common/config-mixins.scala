...
/**
 * Modified Medium Boom. Fetching at most 8 instructions per cycle.
 */
  ...
  case TilesLocated(InSubsystem) => up(TilesLocated(InSubsystem), site) map {
    case tp: BoomTileAttachParams => tp.copy(tileParams = tp.tileParams.copy(core = tp.tileParams.core.copy(
      ...
      newBranchPredictor = ((resp_in: BPBankResponse, p: Parameters) => {
        ...
        ftb.io.resp_in(0)   := fauftb.io.resp
        bim.io.resp_in(0)  := ftb.io.resp
        tage.io.resp_in(0) := bim.io.resp
